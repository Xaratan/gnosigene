...
{
    public class AncestryDNASNPFileReader : IDisposable
    {
        Stream Source { get; }

        StreamReader Reader { get; }

        public AncestryDNASNPFileReader(Stream stream)
        {
            Source = stream;
                ...
                default:
                    {
                        ...
                        var genotypeText = parts[4] + parts[3]; //Add these backwards, since 23andMe does it the other way around
                        ...
                        switch (chromosomeText)
                        {
                            ...
                            case "22":
                                ...
                                break;
                            case "23": //X chromosome
                                humanChromosome = HumanChromosome.ChromosomeX;
                                break;
                            case "24": //Non-pseudoautosomal portion of the Y chromosome
                            case "25": //Pseudoautosomal portion of the Y chromosome
                                humanChromosome = HumanChromosome.ChromosomeY;
                                break;
                            case "26": //Mitochondrial DNA
                                humanChromosome = HumanChromosome.ChromosomeMitochondria;
                                break;
                            default:
                                throw new Exception($"Error in AncestryDNA Raw SNP File. Unknown chromosome '{chromosomeText}'.");
                        }

                        if (!long.TryParse(positionText, out var position))
                        {
                            throw new Exception($"Error in AncestryDNA Raw SNP File. Invalid position '{positionText}'.");
                        }

                        if (genotypeText.Length > 2)
                            throw new Exception($"Error in AncestryDNA Raw SNP File. Invalid genotype '{genotypeText}'.");
